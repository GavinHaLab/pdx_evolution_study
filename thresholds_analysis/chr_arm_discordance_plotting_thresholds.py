...
# -*- coding: utf-8 -*-
"""
Chr Arm Discordance Plotting Thresholds
July 8th, 2020
Anna Hoge
Ha Lab
...
analysis.
--PERCENT_CUTOFF - The percent of bins in a chromosome arm that must be discordant
between two samples for that arm to be called as a discordant.
"""
PERCENT_CUTOFF = 75
USE_ALL_PAIRINGS = True
PLOT_OUTPUT_PATH = "projects/PDX/production/thresholds/plots/chr_arm_discordance/"
SAMPLE_SIZE_THRESHOLDS = [5]
DISCORDANCE_THRESHOLD_PAIRS = [[0.1, 0.3]]
MEDIANS_FILENAME = "chr_arm_medians.txt"
...
--ARM_COORDINATES - List of 1-based numbers indicating the start positions of
each chromosome arm in an hg38 1 Mb binned genome.
"""
...
def discordance_by_passage_comparison(logr_dict, pairings_list, discordance_threshold1,
                                      discordance_threshold2, percent_cutoff,
                                      cohort_name, plot_output_path,
    ...
    lists of the number of chromosome arms discordant or fraction of arms
    discordant for all passage comparisons in the pairings list.  Additionally,
    write the discordance values per pair to the given file, and the white samples to
    the given file.
    """
    all_white_samples = set()
    non_white_checked_samples = set()
    num_arms_discordant_by_passage_comparison_dict = dict()
    ...
    with open(plot_output_path + pairing_discordance_values_filename, "a") as f:
        for pair in pairings_list:
            ...
            if (pair1 not in all_white_samples) and (pair2 not in all_white_samples):
                ...
                num_arms_no_data = 0
                #for each chromosome arm, get the coordinates of its 1Mb bins
                for i in range(len(ARM_COORDINATES) - 1):
                    start = ARM_COORDINATES[i]
                    stop = ARM_COORDINATES[i + 1]
                    chr_arm = is_discordant_list[start - 1:stop - 1] #convert to 0-based numbering
                    #get the length of the arm and the number of gains, losses,
                    #and not discordant values between the two samples
                    num_bins_chr_arm = len(chr_arm)
                    num_gains = chr_arm.count("1")
                    num_losses = chr_arm.count("-1")
                    num_neutral = chr_arm.count("0")
                    #tally if a chromosome arm had no data
                    if num_gains + num_losses + num_neutral == 0:
                        num_arms_no_data += 1
                    #tally if the chromosome arm was gained or lost between the
                    #2 samples, where gains and losses of chromosome arms are defined
                    #as having >= percent_cutoff of the 1 Mb bins discordant
                ...
                percent_genome_discordant = num_arms_discordant / (46 - num_arms_no_data)
                ...
                #to the output file
                num_arms_discordant_by_passage_comparison_dict = update_dict(num_arms_discordant_by_passage_comparison_dict,
                                                                             passage_comparison,
                                                                             num_arms_discordant)
                ...
                f.write(cohort_name + "\t" + pair1 + "\t" + pair2 + "\t" + str(num_arms_discordant) + "\n")
    ...
    return num_arms_discordant_by_passage_comparison_dict, percent_genome_discordant_by_passage_comparison_dict
...
def get_discordance_list(logr_dict, pair1, pair2, discordance_threshold1, discordance_threshold2):
    ...
    a value for each bin in the two input dictionaries.  Values
    of "0" mean the two samples were not discordant at that location, values of
    "1" means there was a CN gain from pair1 to pair2, and values of "-1" means
    there was a CN loss from pair1 to pair2.  "NA" means there was no data for
    that location in the two dictionaries.
    """
    is_discordant_list = []
...
def update_all_white_samples_set(logr_dict, all_white_samples, non_white_checked_samples,
    ...
    if (sample in all_white_samples) or (sample in non_white_checked_samples):
        ...
        num_arms_above_threshold = len([logr for logr in abs_logr_sample_no_na if logr >= 0.3])
        num_arms_data = len(abs_logr_sample_no_na)
        percent_above_threshold = num_arms_above_threshold / num_arms_data
...
def get_discordance_by_cohort(cohort_names, data_tables_filepath, pairings_filepath,
                              ...
                              percent_cutoff, use_all_pairings, plot_output_path,
                              pairing_discordance_values_filename,
                              white_samples_filename):
    """
    Given a list of cohort names, a path to the logR data, and a
    path to the pairings files, returns num_arms_by_cohort_dict and
    ...
    for cohort_name in cohort_names:
        ...
        #get dictionaries of num_arms and percent_genome discordant, where
        #keys specify which passages are being compared and values are lists
        #of discordance values
        num_arms, percent_genome = discordance_by_passage_comparison(logr_dict,
                                                                     pairings_list,
                                                                     discordance_threshold1,
                                                                     discordance_threshold2,
                                                                     percent_cutoff,
                                                                     ...
                                                                     pairing_discordance_values_filename,
                                                                     white_samples_filename)
        num_arms_by_cohort_dict[cohort_name] = num_arms
        percent_genome_by_cohort_dict[cohort_name] = percent_genome
    return num_arms_by_cohort_dict, percent_genome_by_cohort_dict
...
def sorted_discordance_plots(data_tables_filepath, pairings_filepath, cohort_names,
                             bin_size_name, bin_size_abbr, num_arms_ylimit,
                             plot_output_path, sample_size_thresholds, discordance_threshold1,
                             discordance_threshold2, medians_filename, percent_cutoff,
                             use_all_pairings, pairing_discordance_values_filename,
                             white_samples_filename):
    """
    Plots sorted discordance plots, including number of arms discordant
    and fraction genome discordant.
    """
    num_arms_by_cohort_dict, percent_genome_by_cohort_dict = get_discordance_by_cohort(cohort_names,
    ...
    num_arms_pt_passage, num_arms_early_late = break_up_pt_and_passage_comparisons(num_arms_by_cohort_dict)
    ...
    for sample_size_threshold in sample_size_thresholds:
        ...
        plot_discordance_by_cohort(num_arms_pt_passage, cohort_names,
                                   "Number of " + bin_size_name + " Discordant",
                                   "Discordance of PT vs. PDX Passage",
                                   num_arms_ylimit,
                                   "{0}pt_vs_pdx_num_arms_{1}_{2}_or_{3}_diff_threshold_n{4}.png".format(plot_output_path, bin_size_abbr, str(discordance_threshold1), str(discordance_threshold2), str(sample_size_threshold)),
                                   sample_size_threshold, f)
        f.write("\n###Number of " + bin_size_name + " Discordant\n")
        f.write("###Discordance of PDX Passage vs. Later PDX Passage\n")
        plot_discordance_by_cohort(num_arms_early_late, cohort_names,
                                   "Number of " + bin_size_name + " Discordant",
                                   "Discordance of PDX Passage vs. Later PDX Passage",
                                   num_arms_ylimit,
                                   "{0}early_vs_late_num_arms_{1}_{2}_or_{3}_diff_threshold_n{4}.png".format(plot_output_path, bin_size_abbr, str(discordance_threshold1), str(discordance_threshold2), str(sample_size_threshold)),
                                   sample_size_threshold, f)
...
if __name__ == "__main__":
    ...
    for discordance_threshold_pair in DISCORDANCE_THRESHOLD_PAIRS:
        ...
        sorted_discordance_plots(MB_DATA_TABLES_FILEPATH, FILEPATH_PAIRINGS_FILES,
                                 COHORT_NAMES_SELECT, "Chromosome Arms",
                                 "chr_arm{0}".format(PERCENT_CUTOFF), 35,
                                 PLOT_OUTPUT_PATH, SAMPLE_SIZE_THRESHOLDS,
                                 discordance_threshold1, discordance_threshold2,
                                 MEDIANS_FILENAME, PERCENT_CUTOFF,
                                 USE_ALL_PAIRINGS,
                                 PAIRING_DISCORDANCE_VALUES_FILENAME,
                                 WHITE_SAMPLES_FILENAME)
