...
# -*- coding: utf-8 -*-
"""
Discordance Plotting Chr Arm Ichor Minimal Subclone Discordance
July 22nd, 2020
Anna Hoge
Ha Lab
...
PERCENT_CUTOFF = 75
PLOT_OUTPUT_PATH = "projects/PDX/production/ichor_minimal_subclone_discordance/plots/chr_arm_discordance/"
...
def discordance_by_passage_comparison(cn_dict, pairings_list,
                                      additional_discordance_threshold,
                                      percent_cutoff, cohort_name,
    ...
    example) are keys and values are lists of the number of arms discordant or
    fraction of arms discordant for all passage comparisons in the pairings list.
            ...
            if (pair1 not in all_white_samples) and (pair2 not in all_white_samples):
                ...
                for i in range(len(ARM_COORDINATES) - 1):
                    ...
                    if (num_gains / num_bins_chr_arm) >= (percent_cutoff / 100) or (num_losses / num_bins_chr_arm) >= (percent_cutoff / 100):
                        num_bins_discordant += 1
                percent_genome_discordant = num_bins_discordant / (44 - num_bins_no_data)
                ...
                f.write(cohort_name + "\t" + pair1 + "\t" + pair2 + "\t" + str(num_bins_discordant) + "\n")
    ...
    for i in range(len(cn_dict[pair1])):
        ...
        if cn_pair1 == "NA" or cn_pair2 == "NA":
            ...
            if isinstance(cn_pair1, str) and isinstance(cn_pair2, str):
                ...
                discordance = call_clone_subclone_discordance(cn_pair1, cn_pair2)
                is_discordant_list.append(discordance)
            ...
            if pair1_cn_call == pair2_cn_call:
                ...
                if abs(cn_pair1 - cn_pair2) < additional_discordance_threshold:
                    ...
                    #if pair2 value is greater, the CN increased, so label as "1"
                    if cn_pair1 < cn_pair2:
                        is_discordant_list.append("1")
                    #if pair1 is greater, the CN decreased, so label as "-1"
                    if cn_pair1 > cn_pair2:
                        is_discordant_list.append("-1")
...
def call_subclone_subclone_discordance(subclonal_cn_call_1, subclonal_cn_call_2):
    ...
    ranges overlap) or "1" or "-1" indicating whether CN was gained or lost.
    """
    ...
    if (subclone_1_min <= subclone_2_max) and (subclone_2_min <= subclone_1_max):
        return "0"
    #if subclone 2 has a higher, non-overlapping range, CN was gained, so return "1"
    elif subclone_1_max < subclone_2_min:
        return "1"
    #if subclone 1 has a higher, non-overlapping range, CN was lost, so return "-1"
    else:
        return "-1"
...
def call_subclone_clone_discordance(subclonal_cn_call, clonal_cn_call):
    ...
    or "1" or "-1" indicating if CN was gained or lost.
    """
    ...
    #if the subclone's range includes the clonal CN, they are not discordant
    if (subclone_min <= rounded_clonal_cn_call) and (rounded_clonal_cn_call <= subclone_max):
        return "0"
    #if the clonal CN is greater than the subclone's CN range
    elif subclone_max < rounded_clonal_cn_call:
        return "1"
    #if the subclone's CN range is greater than the clonal CN
    else:
        return "-1"
    
    
def call_clone_subclone_discordance(clonal_cn_call, subclonal_cn_call):
    """
    Given a clonal CN call and a subclonal CN call, returns "0" if they are
    not discordant (the range of the subclone call includes the clonal call)
    or "1" or "-1" indicating if CN was gained or lost.
    """
    subclone_min = int(subclonal_cn_call[0])
...
def get_discordance_by_cohort(cohort_names, ichor_results_filepath, pairings_filepath,
                              ...
                              subclone_threshold, percent_cutoff, plot_output_path,
                              pairing_discordance_values_filename,
                              white_samples_filename):
    """
...
def sorted_discordance_plots(ichor_results_filepath, pairings_filepath, cohort_names,
                             ...
                             additional_discordance_threshold,
                             medians_filename, percent_cutoff, use_all_pairings,
...
if __name__ == "__main__":
    sorted_discordance_plots(ICHOR_RESULTS_FILEPATH, FILEPATH_PAIRINGS_FILES,
                             ...
                             USE_ALL_PAIRINGS, USE_ROUNDED_CN,
                             USE_SUBCLONE_FILTER, SUBCLONE_THRESHOLD,
                             PAIRING_DISCORDANCE_VALUES_FILENAME,
                             WHITE_SAMPLES_FILENAME)
