...
plotCNlogRByChr <- function(dataIn, segs, param = NULL, logR.column = "logR", 
  ...
  dataIn$color <- "grey26"
  #dataIn$color[dataIn[, logR.column] < -0.4] <- "blue"
  #dataIn$color[dataIn[, logR.column] > 0.25] <- "red"
  #for one PT sample of interest:
  #dataIn$color[dataIn[, logR.column] > 0.1] <- "red"
  #new scheme:
  dataIn$color[dataIn[, logR.column] < -0.3] <- "blue"
  dataIn$color[dataIn[, logR.column] > 0.3] <- "red"
  ...
  if (!is.null(chr)){
    for (i in chr){
      ...
      plot(coord,as.numeric(dataByChr[, logR.column]),col=dataIn$color,
           pch=16, ylim=yrange,
      ...
      if (!is.null(segs) & plotSegs){
        ...
        apply(segsByChr[ind, ], 1, function(x){
          lines(x[c("start","end")], rep(x["median"], 2), col = dataIn$color, lwd = 3)
          invisible()
        })
        if (sum(!ind) > 0){
          apply(segsByChr[!ind, ], 1, function(x){
    ...
    plot(coord$posns,as.numeric(dataIn[, logR.column]),
         col=dataIn$color,pch=16,xaxt="n", ylim=yrange,
         ...
         main=main,
         panel.first = lines(as.numeric(c(1,coord$posns[length(coord$posns)])),rep(0,2),type="l",col="grey",lwd=2))
